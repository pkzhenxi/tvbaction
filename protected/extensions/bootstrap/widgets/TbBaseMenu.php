...
abstract class TbBaseMenu extends CMenu
{
		...
		if ($n > 0) {
			...
			foreach ($items as $item) {
				...
				if (isset($item['divider'])) {
					...
					if (isset($item['items']) && !empty($item['items'])) {//处理二级菜单
						$dropdownOptions = array(
							'encodeLabel' => $this->encodeLabel,
							'htmlOptions' => isset($item['submenuOptions']) ? $item['submenuOptions'] : $this->submenuHtmlOptions,
							'items' => $item['items'],
						);
						$dropdownOptions['id'] = isset($dropdownOptions['htmlOptions']['id']) ? $dropdownOptions['htmlOptions']['id'] : null;
	...
	{
		if (isset($item['icon'])) {//label是否带icon
		...
		if (!isset($item['linkOptions'])) {//连接样式
			$item['linkOptions'] = array();
		}

		if (isset($item['items']) && !empty($item['items'])) {//item子项目
			if (empty($item['url'])) {
				$item['url'] = '#';
			}
	...
	protected function normalizeItems($items, $route, &$active)
	{
		foreach ($items as $i => $item) {
			if (!is_array($item)) {//如果是空字符,则为一条下划线
				$item = array('divider' => true);
			} else {
				if (!isset($item['itemOptions'])) {//某个item的html属性选项
					$item['itemOptions'] = array();
				}

				$classes = array();

				if (!isset($item['url']) && !isset($item['items']) && $this->isVertical()) {//如果没有相关参数,就设置为标题文字
					$item['header'] = true;
					$classes[] = 'nav-header';
				}

				if (!empty($classes)) {//设置相关item的样式
					$classes = implode($classes, ' ');
